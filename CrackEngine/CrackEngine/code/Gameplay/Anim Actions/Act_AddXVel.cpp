#include "./Act_AddXVel.h"
#include "../../Graphics/AnimManager.h"

Act_AddXVel::Act_AddXVel(pugi::xml_node _xmlData)
{
	init(_xmlData);
	type = "Add X Vel";
}

void Act_AddXVel::execute()
{
...
void Act_AddXVel::execute(int _stepCount)
{
	switch (frequency) {
...
void Act_AddXVel::execute(int _stepCount, AnimManager* _manager)
{
	switch (frequency) {
	case animActionFrequency::once:
		if (_stepCount == stepCount)
		{
			_manager->setxVelocity(xVel, true);
		}
		break;
	case animActionFrequency::perFrameChange:
		if (_stepCount == 0)
...
void Act_AddXVel::init(pugi::xml_node _xmlData)
{
	// get data from xml
