...
// Game constants (board size comes from header)
...
// 6 preview sprites to show the next 2 sets of blocks
...
void ColumnsBoard::update() {
	...
	case GameState::FALLING:
	{
		...
		if (controls->getButton(controls->SOUTH) == true)
		{
			...
			// If player is holding down and the active piece is on the ground,
			// end the lock delay so it will lock immediately.
			if (isActivePieceOnGround()) {
				lockDelayCount = ColumnsConfig::getInstance().getLockDelay();
			}
		}
		if (input == false)
...
void ColumnsBoard::updatePreviewGraphics()
{
	// Update preview sprites animations based on previewValues
	for (int set = 0; set < 2; ++set)
	{
		for (int slot = 0; slot < 3; ++slot)
		{
			if (previewGraphics[set][slot])
			{
				previewGraphics[set][slot]->update();
			}
		}
	}
}
...
void ColumnsBoard::render() {
	...
	for (int set = 0; set < 2; ++set)
	{
		for (int slot = 0; slot < 3; ++slot)
		{
			if (previewGraphics[set][slot])
			{
				previewGraphics[set][slot]->render();
			}
		}
	}
	foreground->render();
...
{
	state = newState;
	// reset all timers and counters as needed
	frameCount = 0;
	lockDelayCount = 0;
	spawnDelayCount = 0;
	// reset active piece tracking
	for (int i = 0; i < BOARD_HEIGHT; i++)
	{
		for (int j = 0; j < BOARD_WIDTH; j++)
		{
		...
		for (int j = 0; j < BOARD_WIDTH; j++) {
			if (playBoardValues[i][j] != GridValue::EMPTY) {
				if (ColumnsConfig::getInstance().getVerticalChaining()) { // Vertical chaining
					...
					currentY++;
					while (matching == true) {
						// stop before reading beyond last valid row
				...
				if (ColumnsConfig::getInstance().getHorizontalChaining()) { // Horizontal chaining
					...
					currentX++;
					while (matching == true) {
						// stop before reading beyond last valid column
				...
				if (ColumnsConfig::getInstance().getDiagonalChaining()) { // Forward slash chaining
					...
					currentY++;
					while (matching == true) {
						// stop before reading beyond bounds
...
bool ColumnsBoard::checkLostGame()
{
	// Check if any blocks are in the top row
	for (int j = 0; j < BOARD_WIDTH; j++)
	{
		if (playBoardValues[BOARD_HEIGHT - 3][j] != GridValue::EMPTY && playBoardActiveValues[BOARD_HEIGHT - 3][j] == false)
		{
			return true;
		}
	}
...
void ColumnsBoard::initializeBoard() {
	...
	for (int set = 0; set < 2; ++set)
	{
		for (int slot = 0; slot < 3; ++slot)
		{
			float px = previewLeft + (set * (spriteWidth + 10.f));
			float py = previewBottom - (slot * spriteWidth);
			previewGraphics[set][slot] = new AnimManager((std::string)"data/characters/RedPiece.xml", glm::vec3(px, py, 0.f));
			previewValues[set][slot] = GridValue::EMPTY;
			resetPreviewSprite(set, slot);
		}
	}
	spawnNewBlocks();
}
...
void ColumnsBoard::resetPreviewSprite(int setIndex, int slotIndex)
{
	if (previewGraphics[setIndex][slotIndex] != nullptr)
	{
		GridValue val = previewValues[setIndex][slotIndex];
		int desiredAnim = 1;
		switch (val)
		{
		case GridValue::EMPTY: desiredAnim = 1; break;
		case GridValue::GREEN: desiredAnim = 2; break;
		case GridValue::ORANGE: desiredAnim = 3; break;
		case GridValue::RED: desiredAnim = 4; break;
		case GridValue::BLUE: desiredAnim = 5; break;
		case GridValue::PURPLE: desiredAnim = 6; break;
		case GridValue::TEAL: desiredAnim = 7; break;
		default: desiredAnim = 1; break;
		}
		if (previewGraphics[setIndex][slotIndex]->currentAnim == nullptr || previewGraphics[setIndex][slotIndex]->currentAnim->index != desiredAnim)
		{
			previewGraphics[setIndex][slotIndex]->changeAnimation(desiredAnim);
		}
	}
}
...
void ColumnsBoard::spawnNewBlocks() {
	...
	for (int i = 0; i < 3; i++) {
		if (bagIndex >= (int)randomBag.size())
		{
			bagIndex = 0;
		}
		int y = startY - i;
		playBoardValues[y][spawnX] = randomBag[bagIndex];
		playBoardActiveValues[y][spawnX] = true;
		resetPanelSprite(y, spawnX);
		bagIndex++;
	}
	// Fill preview with next 2 sets (6 items from bagIndex)
	int previewIdx = bagIndex;
	for (int set = 0; set < 2; ++set)
	{
		for (int slot = 0; slot < 3; ++slot)
		{
			if (previewIdx >= (int)randomBag.size()) previewIdx = 0;
			previewValues[set][slot] = randomBag[previewIdx];
			resetPreviewSprite(set, slot);
			previewIdx++;
		}
	}
...
// Helper: returns true if there is an active piece and none of its active cells have empty space below
bool ColumnsBoard::isActivePieceOnGround()
{
	...
	for (int y = 0; y < BOARD_HEIGHT; ++y)
	{
		for (int x = 0; x < BOARD_WIDTH; ++x)
		{
			if (playBoardActiveValues[y][x])
			{
				foundActive = true;
				// if there's space below (y-1) and it's empty, not grounded
				if (y - 1 >= 0 && playBoardValues[y - 1][x] == GridValue::EMPTY)
				{
					return false;
				}
				// if y == 0 -> on bottom row, grounded for that cell
			}
		}
	}
	return foundActive;
